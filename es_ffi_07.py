...
def isprime(n: int) -> bool:
    ...
    # return all(n % potenziale_divisore for potenziale_divisore in range(3, int(n**(1/2))+1, 2))
...
def isprime_old(n: int) -> bool:
    ...
    return all(n % potenziale_divisore for potenziale_divisore in range(3, n//2, 2))
...
def gen_primi(limite_sup: int, limite_inf: int = 2):
    ...
    n = limite_inf+(limite_inf % 2)-1 #
                                      #
    if limite_inf <= 2:               # in realtà non serve, figlio di uno ragionamento
        yield 2                       #
        n = 3                         #
...
def somme_pari_primi_lento(numero: int):
    ...
    # ans = [str(i)+'+'+str(j) for i in gen_primi(numero//2+1) for j in gen_primi(numero, numero-i-10) if i+j == numero]
    set_primi=set(gen_primi(numero))
    ans = [str(i)+'+'+str(numero-i)
           for i in set_primi if numero-i in set_primi]

    '''for i in gen_primi(numero//2+1):
            if isprime(numero-i):
                ans.append(str(i)+'+'+str(numero-i))'''
